...
默认输出一套“中密度”成品，不需要用户额外要求逐张制作。上传 1 张照片时生成 1 张单图拼贴；上传 2–8 张照片时，先为每张原图分别生成 1 张单图拼贴，再生成 1 张包含全部原图的总结拼贴。因此上传 6 张照片时默认输出 7 张成品：6 张单图和 1 张六图合并图。所有成品统一为严格的 **3:4 竖版比例**，只有额外风格版本需要用户明确提出。
